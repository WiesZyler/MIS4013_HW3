...
function selectDecksWithCards($did) {
    try {
        ...
        $stmt = $conn->prepare("SELECT cardname, cardnumber, cardtype, d.deckID, deckname, duelistname, c.packID, packName FROM `pack` p JOIN `card` c ON p.packID = c.packID JOIN `card/deck` cd ON c.cardID = cd.cardID JOIN `deck` d ON cd.deckID = d.deckID WHERE d.deckID = ?");
