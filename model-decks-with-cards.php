...
function selectDecksWithCards($did) {
    try {
        ...
        $stmt = $conn->prepare("SELECT cardID, cardnumber, cardname, attribute, `spell/trap`, level, monstertype, rarity, cardtype, d.deckID, deckname, duelistname, quantity FROM `card` c JOIN `card/deck` cd ON c.cardID = cd.cardID JOIN `deck` d ON cd.deckID = d.deckID WHERE d.deckID = ?");
